...
public static class ParticlePool
{
    ...
    // All of our pools
    static Dictionary<EntityId, Pool> pools = new Dictionary<EntityId, Pool>();
    ...
    {
        if (prefab != null && !pools.ContainsKey(prefab.GetEntityId()))
        {
            pools[prefab.GetEntityId()] = new Pool(prefab, qty, parent);
        }
    }
    ...
    static public void Play(ParticleSystem prefab, Vector3 pos, Quaternion rot)
    {
        ...
        if (!pools.ContainsKey(prefab.GetEntityId()))
        {
            Transform newRoot = new GameObject("VFX_" + prefab.name).transform;
            newRoot.SetParent(Root);
            pools[prefab.GetEntityId()] = new Pool(prefab, 10, newRoot);
        }

        pools[prefab.GetEntityId()].Play(pos, rot);
    }
    ...
    static public void Release(ParticleSystem prefab)
    {
        if (pools.ContainsKey(prefab.GetEntityId()))
        {
            pools[prefab.GetEntityId()].Release();
            pools.Remove(prefab.GetEntityId());
        }
        else
        {
